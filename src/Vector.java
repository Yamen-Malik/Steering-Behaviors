public class Vector{
     ...
     * @param angle the angle between the vector and the +x axis (in degrees)
     */
    public Vector(double mag, double angle){
        Setup(mag, angle, false);
    }
    /**
     * @param mag       the magnitude of the vector
     * @param angle     The angle of the vector relative to x+ axis
     * @param isRadians if the value is false, uses the given angle in degrees (true -> radians)
     */
    public Vector(double mag, double angle, boolean isRadians){
        Setup(mag, angle, isRadians);
    }
    /**
     * Creates a vector and copys the properties of a given vector
     * 
    ...
    void Setup(double mag, double angle, boolean isRadians){
        if (mag == 0){
    ...
    //#endregion
    //#region getters
    public double getAngleInDegrees(){ return Math.toDegrees(angle); }
    public double getAngleInRadians(){ return angle; }
    public double getMag(){ return mag; }
    public double getXMag(){ return xMag; }
    public double getYMag(){ return yMag; }
    //#endregion
    ...
    private static Double CalculateMag(double xMag, double yMag){ 
        return Math.pow(Math.pow(xMag, 2) + Math.pow(yMag, 2),0.5);
    }
